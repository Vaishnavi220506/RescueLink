...
const offerStatuses = ['ACTIVE', 'PAUSED', 'EXHAUSTED', 'EXPIRED'] as const;
const hazardTypes = ['FLOOD', 'FIRE', 'ROAD_BLOCK', 'POWER_LINE', 'DEBRIS', 'BUILDING_DAMAGE', 'OTHER'] as const;
const severities = ['INFO', 'LOW', 'MEDIUM', 'HIGH', 'CRITICAL'] as const;
const verifications = ['UNVERIFIED', 'COMMUNITY_VERIFIED', 'ADMIN_VERIFIED', 'REJECTED'] as const;
...
export const hazardSchema = z.object({ type: z.enum(hazardTypes), description: z.string().trim().min(15).max(2000), severity: z.enum(['LOW', 'MEDIUM', 'HIGH', 'CRITICAL']), locationLabel: z.string().trim().min(2).max(180), lat: latitude, lng: longitude });
...
export const listSchema = z.object({ page: z.coerce.number().int().min(1).default(1), limit: z.coerce.number().int().min(1).max(100).default(20), status: z.enum(statuses).optional(), offerStatus: z.enum(offerStatuses).optional(), urgency: z.enum(urgencies).optional(), category: z.enum(categories).optional(), type: z.enum(hazardTypes).optional(), severity: z.enum(severities).optional(), verification: z.enum(verifications).optional(), lat: z.coerce.number().min(-90).max(90).optional(), lng: z.coerce.number().min(-180).max(180).optional(), radius: z.coerce.number().int().min(100).max(100000).default(10000) }).superRefine((value, context) => { if ((value.lat === undefined) !== (value.lng === undefined)) context.addIssue({ code: z.ZodIssueCode.custom, message: 'Latitude and longitude must be provided together.', path: ['lat'] }); });
export const liveLocationSchema = z.object({ lat: latitude, lng: longitude, status: z.string().trim().min(2).max(40), note: z.string().trim().max(240).optional(), ttlMinutes: z.number().int().min(5).max(15).optional() });
export const moderationSchema = z.object({ verification: z.enum(['ADMIN_VERIFIED', 'REJECTED']) });
export const notificationParamsSchema = z.object({ id: z.string().trim().min(1).max(100) });
